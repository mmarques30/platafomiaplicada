...
export function BusinessProjetosSection({ 
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { label: string; className: string }> = {
      ...
      'concluido': { label: 'Concluído', className: 'bg-status-success/10 text-status-success border-status-success/30' },
      'pendente': { label: 'Pendente', className: 'bg-muted text-muted-foreground border-border' },
      'pausado': { label: 'Pausado', className: 'bg-status-danger/10 text-status-danger border-status-danger/30' },
    };
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-foreground flex items-center gap-2 text-lg font-semibold">
              <FolderKanban className="h-5 w-5 text-brand-strong" />
                      ...
                      <div className="border-t border-border p-4 space-y-3">
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <Button 
                            ...
                            className="h-7 text-xs text-brand-strong hover:text-brand-strong hover:bg-brand-strong/15"
                          >
                              ...
                              >
                                ...
                                {tarefa.arquivo_entrega_url ? (
                                  ...
                                  <label className="cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    />
                                    <Upload className="h-4 w-4 text-muted-foreground hover:text-brand-strong" />
                                  </label>
                                )}
                              </div>
                            ))}
