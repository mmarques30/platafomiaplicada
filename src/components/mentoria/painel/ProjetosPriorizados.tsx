    ...
    <Card className="bg-card text-card-foreground border border-border">
      ...
      <CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-primary hover:bg-primary">
                ...
                <TableHead className="text-primary-foreground font-bold">Objetivo</TableHead>
                <TableHead className="text-primary-foreground font-bold">Complexidade</TableHead>
                <TableHead className="text-primary-foreground font-bold">Prazo</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {projetos.map((projeto, index) => (
                <TableRow 
                  key={projeto.id}
                  className={index % 2 === 0 ? "bg-card hover:bg-muted" : "bg-muted hover:bg-muted/80"}
                >
                  <TableCell className="font-bold text-card-foreground">{index + 1}</TableCell>
                  <TableCell className="font-semibold text-card-foreground">{formatProjetoTitulo(projeto.titulo)}</TableCell>
                  <TableCell className="text-card-foreground">{projeto.objetivo_projeto}</TableCell>
        ...
        <div className="md:hidden space-y-3">
          {projetos.map((projeto, index) => (
            <Card key={projeto.id} className="bg-card border border-border">
              <CardContent className="pt-4">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <h3 className="font-semibold text-card-foreground flex-1">{formatProjetoTitulo(projeto.titulo)}</h3>
                </div>
                <div className="space-y-2 text-sm">
                  <div>
                    <span className="text-muted-foreground font-medium">Objetivo:</span>
                    <p className="text-card-foreground mt-1">{projeto.objetivo_projeto}</p>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground font-medium">Complexidade:</span>
                    {getComplexityBadge(index)}
                  </div>
                  {projeto.data_entrega && (
                    <div>
                      <span className="text-muted-foreground font-medium">Prazo:</span>
                      <p className="text-card-foreground mt-1 font-medium">{new Date(projeto.data_entrega).toLocaleDateString('pt-BR')}</p>
                    </div>
                  )}
                </div>
              </CardContent>
        ...
        {/* Impacto Total */}
        <Card className="mt-6 bg-muted border border-border">
          <CardContent className="pt-6">
            <h3 className="text-lg font-bold text-foreground mb-2">Impacto Total Estimado</h3>
