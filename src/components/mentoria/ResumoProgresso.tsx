...
export function ResumoProgresso() {
  ...
  const objetivosCompletos = objetivos.filter(o => o.status === "concluido").length;
  const progressoObjetivos = objetivos.length > 0 ? (objetivosCompletos / objetivos.length) * 100 : 0;
  ...
  const progressoGeral = ((diagnosticoCompleto + (objetivos.length > 0 ? 1 : 0) + (tarefas.length > 0 ? 1 : 0)) / 3) * 100;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          <ProgressRing
            ...
            label="Objetivos"
            subtitle={`${objetivosCompletos} de ${objetivos.length}`}
