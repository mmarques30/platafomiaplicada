-- CORREÇÃO COMPLETA: Limpar duplicações e criar trilha única

-- ETAPA 1: Deletar TODAS as trilhas "Planilhas & Dados com IA" duplicadas
-- Primeiro deletar os módulos associados
DELETE FROM modulos 
WHERE trilha_id IN (
  SELECT id FROM trilhas WHERE titulo = 'Planilhas & Dados com IA'
);

-- Depois deletar as trilhas
DELETE FROM trilhas 
WHERE titulo = 'Planilhas & Dados com IA';

-- ETAPA 2: Reorganizar ordens das trilhas restantes (normalizar sequência)
-- Garantir que não há gaps na sequência de ordens
...
UPDATE trilhas t
SET ordem = tord.nova_ordem
FROM trilhas_ordenadas tord
WHERE t.id = tord.id;

-- ETAPA 3: Abrir espaço na posição 2 para a nova trilha
UPDATE trilhas 
SET ordem = ordem + 1 
WHERE ordem >= 2;

-- ETAPA 4: Criar UMA ÚNICA trilha "Planilhas & Dados com IA"
...
-- ETAPA 5: Inserir os 10 módulos únicos usando o ID da trilha recém-criada
WITH nova_trilha AS (
