-- DROP e recriar get_ranking_engajamento REMOVENDO total_videos_assistidos
DROP FUNCTION IF EXISTS public.get_ranking_engajamento();

CREATE OR REPLACE FUNCTION public.get_ranking_engajamento()
 RETURNS TABLE(user_id uuid, nome_completo text, avatar_url text, total_pontos integer, posicao integer, total_posts bigint, total_comentarios bigint, total_likes_dados bigint, total_likes_recebidos bigint, dias_ativos_30d bigint)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT 
    ...
    us.nome_completo,
    us.avatar_url,
    -- CÁLCULO SEM VÍDEOS: apenas engajamento social + dias ativos
    (
      (us.posts * 20) + 
      (us.comentarios * 5) + 
      (us.likes_dados * 2) + 
